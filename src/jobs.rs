...
pub async fn publish_readme_files(
    ...
    .await?;
    let keys = crate::storage::s3::get_objects_in_bucket(configuration, Some("crates")).await?;
    for crate_data in crates {
        ...
        for version in versions {
            ...
            let storage = crate::storage::get_storage(configuration);
            let data = storage.download_crate(&crate_data.name, &version.vers).await?;
            let readme = crate::storage::extract_readme(&data)?;
            storage.store_crate_readme(&crate_data.name, &version.vers, readme).await?;
